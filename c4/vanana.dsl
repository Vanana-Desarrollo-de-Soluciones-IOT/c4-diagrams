workspace "Vanana Platform" "High-level architecture for smart device management." {
    ...
    model {
        ...
        vanana = softwareSystem "Vanana Platform" "Central platform for monitoring, controlling, and automating IoT devices." {
            ...
            clairEmbeddedApp = container "Clair Embedded Application" "ESP32 firmware implementing ModestIoT framework with Event/Command architecture." "C++" "Embedded" {
                eventBus = component "Event Bus" "In-memory event propagation channel. Sensors raise events, Device orchestrator consumes them." "EventHandler" "ModestIoT"
                commandBus = component "Command Bus" "In-memory command dispatch channel. Edge commands are routed to Device and Actuators." "CommandHandler" "ModestIoT"
                deviceOrchestrator = component "Device Orchestrator" "Central coordinator implementing Device interface. Subscribes to events, executes commands, manages state machine (Init/Standby/Normal)." "ClairDevice" "ModestIoT"
                co2Sensor = component "CO2/T/H Sensor" "Raises DATA_READY events via Event Bus when new measurements are available." "SCD41Sensor" "ModestIoT"
                pmSensor = component "Particulate Matter Sensor" "Raises DATA_READY events via Event Bus when new PM1.0/2.5/10 readings are ready." "PMS5003Sensor" "ModestIoT"
                domainAggregator = component "Domain Aggregator" "Pure domain logic: evaluateStatus() from Event, calculateAQI(), threshold enforcement. No hardware dependencies." "ClairData + AirQualityThresholds" "ModestIoT"
                telemetryPublisher = component "Telemetry Publisher" "Outbound adapter. Listens to device state, pushes to Edge via HTTPS on interval or force command." "EdgeService" "ModestIoT"
                commandPoller = component "Command Poller" "Inbound adapter. Polls Edge for pending commands, dispatches to Command Bus." "EdgeService + RemoteCommand" "ModestIoT"
                incidentPoller = component "Incident Poller" "Inbound adapter. Polls Edge for active alerts, updates incident state via Command Bus." "IncidentManager" "ModestIoT"
                ledActuator = component "LED Actuator" "Responds to LED_ON, LED_OFF, LED_BLINK commands. Updates physical GPIO." "Led" "ModestIoT"
                displayActuator = component "Display Actuator" "Responds to DISPLAY_ON/OFF/SLEEP/WAKE commands. Updates OLED via I2C." "OLEDDisplay" "ModestIoT"
            }
        ...
        co2Sensor -> eventBus  "Raises DATA_READY_EVENT"
        pmSensor -> eventBus  "Raises DATA_READY_EVENT"
        eventBus -> deviceOrchestrator  "Delivers events to on() handler"
        commandPoller -> commandBus  "Dispatches remote commands (STANDBY, WAKE, REPORT)"
        commandBus -> deviceOrchestrator  "Routes commands to handle()"
        commandBus -> ledActuator  "Routes LED commands"
        commandBus -> displayActuator  "Routes display commands"
        deviceOrchestrator -> domainAggregator  "Calls evaluateStatus() / calculateAQI()"
        domainAggregator -> deviceOrchestrator  "Returns updated status"
        deviceOrchestrator -> telemetryPublisher  "Publishes ClairData on interval/force"
        telemetryPublisher -> edgeDeviceContext   "POST /telemetry (every 10s)"
        commandPoller -> edgeDeviceContext  "GET /commands/pending (every 10s) & POST /commands/{id}/ack"
        incidentPoller -> edgeAlertingContext  "GET /incidents/pending (every 5s) & POST /incidents/{id}/ack"
        incidentPoller -> deviceOrchestrator  "Updates incident state via hasActiveIncidents()"
        deviceOrchestrator -> ledActuator  "Calls startBlink() / off() based on incidentManager.hasActiveIncidents()"
        ledActuator -> hardware  "Controls via GPIO"
        displayActuator -> hardware  "Controls via I2C"
        co2Sensor -> hardware  "Reads via I2C (SCD41)"
        pmSensor -> hardware  "Reads via UART (PMS5003)"
